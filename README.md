...
Our filtering isn't pefect, and there are papers that should be in our subsets, that aren't. 

#### Step 2: Prune, and creating embeddings dataset

Each paper has a list of references and citations. We only want papers that have a citation or reference to one of the other papers in our dataset (otherwise its embedding will never get a chance to be trained on in word2vec), so we prune at papers that do not have that. Next, we map a unique embedding ID for each paper, save the next data, and create a HDF5 dataset to be used for word2vec training. 
...
#### Step 4: Creat Bert Dataset.

After word2vec training, we have an embedding which represents each paper. We can use this vector as a label for the mean-pooled output of the last hidden states of Bert, the input being each paper's abstract. We use the SciBert vocab for the tokenizer. SciBert was actually trained on many of the papers in the Semantic Scholar Corpus. 
...
Our plans are to eventually run inference on all 179 million papers on the Semantic Scholar Corpus, which is a ton of papers to run cos similarity on. So one of the members Srihari created a new library for running cos similarity are large numbers of embeddings **over a TPU**!.
